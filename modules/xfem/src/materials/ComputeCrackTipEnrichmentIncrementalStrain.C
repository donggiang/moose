...
ComputeCrackTipEnrichmentIncrementalStrain::computeProperties()
{
  ...
  for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
  {
    ...
    //   _small_strain[_qp] = 0.5 * ((_grad_disp_tensor[_qp]+_grad_enrich_disp_tensor[_qp]) +
    //   (_grad_disp_tensor[_qp].transpose()+_grad_enrich_disp_tensor[_qp]).transpose());
    // //  _small_strain[_qp] = 0.5 * ((_grad_disp_tensor[_qp]) )+ (_grad_disp_tensor[_qp].transpose());
    ...
    //     (_grad_disp_tensor_old[_qp]+_grad_enrich_disp_tensor_old[_qp]))/_dt;
    //   //   _grad_disp_rate[_qp] = ((_grad_disp_tensor[_qp]) - (_grad_disp_tensor_old[_qp]))/_dt/1.0;

    // if (_volumetric_locking_correction)
    //   volumetric_strain += _strain_increment[_qp].trace() * _JxW[_qp] * _coord[_qp];
  }
  // if (_volumetric_locking_correction)
