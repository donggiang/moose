    ...
    _dBx(4)
{
  ...
  _nlb = &(_fe_problem.getNonlinearSystemBase(0));
...
void
ComputeCrackTipEnrichmentIncrementalStrain::initialSetup()
{
...
ComputeCrackTipEnrichmentIncrementalStrain::computeProperties()
{
  ...
  // const Point * crack_front_point = _mesh_cut_uo._crack_front_definition->getCrackFrontPoint(1);
  // // std::cout<< "ComputeCrackTipEnrichmentIncremental: " << *crack_front_point << "\n";
  ...
  //  std::cout<<  _nl->system().current_local_solution;
  // std::cout<< _nl->solutionOld()(50) <<"\n";
  //  Vector<Real>  old_sln=_nl->solutionOld();

  // std::cout<< "ComputeCrackTipEnrichmentsolution, current \n";
  // std::cout<< _sln << "\n";
  // std::cout<< "ComputeCrackTipEnrichmentsolution,  old \n";
  // std::cout<< _old_sln << "\n";
  // std::cout<< "ComputeCrackTipEnrichmentsolution,  old , after\n";
  // _old_sln(dof_id) =
  //   std::cout<< "ComputeCrackTipEnrichment, solution size: " << _sln->size() << "\n";
  //  std::cout<< "ComputeCrackTipEnrichment, element id: " << _current_elem->id() <<", " <<
  //  *_current_elem<< "\n";
  ...
  for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
  {
    ...
    // {
    //     std::cout<< "ComputeCrackTipEnrichmentIncrementalStrain before**********************\n";
    //     std::cout<< "current solution: \n";
    //     std::cout<< *_nl->currentSolution()<<"\n";
    //     std::cout<< "old solution: \n";
    //     std::cout<< _nl->solutionOld()<<"\n";
    ...
    for (unsigned int m = 0; m < _ndisp; ++m)
    {
      ...
      for (unsigned int i = 0; i < _current_elem->n_nodes(); ++i)
      {
        ...
        for (unsigned int j = 0; j < 4; ++j)
        {
          ...
          //      ((*_fe_dphi)[i][_qp] * (_B[j] - _BI[i][j]) + (*_fe_phi)[i][_qp] * grad_B) *
          //      soln_old;
        }
      }
    }
    ...
    // //if(_qp==0 &&_current_elem->id()==0)
    // {
    //   // std::cout<< "_qp=" << _qp << ", " << "strain norm2: " << _small_strain[_qp].norm() << ", " << _grad_disp_tensor[_qp].norm()
    //   // << ", " << _grad_enrich_disp_tensor[_qp].norm()<<  "\n";
    //   if(std::abs(_grad_enrich_disp_tensor[_qp].norm()/_grad_disp_tensor[_qp].norm())>500.0 &&
    //   _t>2*_dt)
    //   {
    //     //std::cout<< "enrich strain is jumped, force it to the old value !!!\n";
    //     _grad_enrich_disp_tensor[_qp] =_grad_enrich_disp_tensor_old[_qp];
    //     // for (unsigned int m = 0; m < _ndisp; ++m)
    //     // {
    //     //   _enrich_disp[m] = 0.0;
    //     //   _grad_enrich_disp[m].zero();
    //     //   for (unsigned int i = 0; i < _current_elem->n_nodes(); ++i)
    //     //   {
    //     //     const Node * node_ii = _current_elem->node_ptr(i);
    //     //     for (unsigned int j = 0; j < 4; ++j)
    //     //     {
    //     //       dof_id_type dof = node_ii->dof_number(_nl->number(), _enrich_variable[j][m]->number(), 0);
    //     //       Real soln1 = (*_sln)(dof);
    //     //       std::cout<< "soln1: " << soln1 << "\n";
    //     //       // _enrich_disp[m] += (*_fe_phi)[i][_qp] * (_B[j] - _BI[i][j]) * soln;
    //     //       // RealVectorValue grad_B(_dBX[j]);
    ...
    // RankTwoTensor small_strain_old = 0.5 * ((grad_disp_tensor_old+grad_enrich_disp_tensor_old)
    //                        + (grad_disp_tensor_old+grad_enrich_disp_tensor_old).transpose());

    _strain_increment[_qp] = _small_strain[_qp] - _small_strain_old[_qp];
    // _strain_increment_no_enrich[_qp] = 0.5 * (_grad_disp_tensor[_qp] +
    // _grad_disp_tensor[_qp].transpose())
    //                                      - 0.5 * (_grad_disp_tensor_old[_qp] +
    //                                      _grad_disp_tensor_old[_qp].transpose());

    //   _grad_disp_rate[_qp] = ((_grad_disp_tensor[_qp]) - grad_disp_tensor_old)/_dt;
    _grad_disp_rate[_qp] = ((_grad_disp_tensor[_qp] + _grad_enrich_disp_tensor[_qp]) -
                            (_grad_disp_tensor_old[_qp] + _grad_enrich_disp_tensor_old[_qp])) /
                           _dt / 1.0;
...
void
ComputeCrackTipEnrichmentIncrementalStrain::computeTotalStrainIncrement(
