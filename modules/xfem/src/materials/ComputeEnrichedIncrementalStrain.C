...
#include "ComputeEnrichedIncrementalStrain.h"
#include "MooseMesh.h"
...
registerMooseObject("XFEMApp", ComputeEnrichedIncrementalStrain);
registerMooseObject("XFEMApp", ADComputeEnrichedIncrementalStrain);

template <bool is_ad>
InputParameters
ComputeEnrichedIncrementalStrainTempl<is_ad>::validParams()
{
...
template <bool is_ad>
ComputeEnrichedIncrementalStrainTempl<is_ad>::ComputeEnrichedIncrementalStrainTempl(
  ...
  : ComputeIncrementalStrainBaseParent<is_ad>(parameters),
    ...
    _grad_disp_tensor(
        this->template declareGenericProperty<RankTwoTensor, is_ad>("grad_disp_tensor")),
    _small_strain(this->template declareGenericProperty<RankTwoTensor, is_ad>("small_strain")),
    _grad_enrich_disp_tensor(
        this->template declareGenericProperty<RankTwoTensor, is_ad>("grad_enrich_disp_tensor")),
    _grad_disp_tensor_old(this->template getMaterialPropertyOld<RankTwoTensor>("grad_disp_tensor")),
    _small_strain_old(this->template getMaterialPropertyOld<RankTwoTensor>("small_strain")),
    _grad_enrich_disp_tensor_old(
        this->template getMaterialPropertyOld<RankTwoTensor>("grad_enrich_disp_tensor")),
    _B(4),
    _dBX(4),
    _dBx(4)
{
...
template <bool is_ad>
void
ComputeEnrichedIncrementalStrainTempl<is_ad>::initialSetup()
{
  const auto uo_name = this->template getParam<UserObjectName>("crack_front_definition");
  _crack_front_definition =
      &(this->_fe_problem.template getUserObject<CrackFrontDefinition>(uo_name));
}

template <bool is_ad>
void
ComputeEnrichedIncrementalStrainTempl<is_ad>::computeProperties()
{
