#!/usr/bin/env python3
"""DCB CZM wedge-mesh CG solutions at two mesh refinements (h=0.5 mm,
h=0.25 mm) overlaid on experimental data and beam-theory landmarks.
"""
import csv
...
def main():
    ...
    runs = [
        ("Cubit wedge h = 0.5 mm",  "tier2_1_dcb_cg_davila_wedge_h0p5_out.csv",
         {"color": "tab:purple", "marker": "^"}),
        ("Cubit wedge h = 0.25 mm", "tier2_1_dcb_cg_davila_wedge_h0p25_out.csv",
         {"color": "tab:orange", "marker": "s"}),
        ("MOOSE 2-block (gen, h = 0.5 mm)", "tier2_1_dcb_cg_davila_2blk_out.csv",
         {"color": "tab:blue",   "marker": "D"}),
    ]
    sims = []
    for label, fname, style in runs:
        ...
        sims.append({'label': label, 'style': style, 'opening': opening, 'P': P,
                     ...
                     'jump_final': d['max_normal_jump'][-1]})
        print(f"{label:<24} peak {peak:>5.2f} N at opening {peak_open:>4.2f} mm "
              f"(final opening {opening[-1]:.2f} mm, final P {P[-1]:.2f} N, "
              f"final max_jump {d['max_normal_jump'][-1]:.2e} mm)")
    ...
    print(f"Experiment              peak {exp_peak:.2f} N at opening "
          f"{exp_peak_open:.2f} mm")
    ...
    ax.set_title("Tier 2.1 DCB CZM CG -- Cubit wedge vs MOOSE 2-block vs experiment")
    ax.grid(alpha=0.3)
    ...
    fig.tight_layout()
    out = here / "tier2_1_cubit_vs_moose.png"
