...
RadialReturnCreepStressUpdateBaseTempl<is_ad>::RadialReturnCreepStressUpdateBaseTempl(
  ...
  : RadialReturnStressUpdateTempl<is_ad>(parameters),
    ...
    _creep_rate(this->template declareGenericProperty<Real, is_ad>("creep_rate"))
{
    ...
    const GenericMaterialProperty<RankTwoTensor, is_ad> & /**/)
{
  ...
  _creep_rate[_qp] = computeCreepStrainRate(sigma_eq);

  // const GenericReal<is_ad> eps_eq =
  //     std::sqrt(2.0 / 3.0 * strain_rate[_qp].doubleContraction(strain_rate[_qp]));

  Real integral = MetaPhysicL::raw_value(sigma_eq * _creep_rate[_qp]);
